...
# 2) Halving & wave definitions (as before)
...
for t in idx:
    ...
    tr_arr.append(tr); up_arr.append(up)
    dn_arr.append(dn); mid_arr.append(mid)
...
# Main (row1): vertical RSI shading + wave + price + stop
...
fig.add_trace(go.Scatter(x=idx, y=tr_arr,   mode='lines', line=dict(color='red', dash='dash'), name='Trend'), row=1, col=1)
...
for m in midpoints: fig.add_vline(x=m, line=dict(color='gray', dash='dash'))

# RSI subplot (row2)
fig.add_trace(go.Scatter(x=rsi.index, y=rsi, mode='lines', line=dict(color='black'), name='RSI'), row=2, col=1)

# 9) Layout: slider only on RSI (xaxis2), linear y-axis
fig.update_layout(
    height=700,
    hovermode='x unified',
    xaxis=dict(rangeslider=dict(visible=False), type='date'),
    xaxis2=dict(rangeslider=dict(visible=True), type='date', overlaying="x", row=2),
