# Streamlit App: BTC Wave Model + RSI-Proportional Vertical Shading (No Slider)
...
# 7a) Vertical RSI-based shading on main chart
max_opacity = 0.3
for dt, val in rsi.items():
    if val > 50:
        # opacity scales from 0 at 50 to max_opacity at 100
        op = ((val - 50)/50) * max_opacity
        fig.add_vrect(x0=dt, x1=dt + timedelta(days=1),
                      y0=0, y1=1,
                      fillcolor="red", opacity=op,
                      layer="below", row=1, col=1, yref="paper")

# 7b) Main chart traces
fig.add_trace(go.Scatter(x=idx, y=ups, line=dict(color='rgba(0,0,0,0)'), showlegend=False), row=1, col=1)
fig.add_trace(go.Scatter(x=idx, y=dns, fill='tonexty', fillcolor='rgba(0,0,200,0.2)',
                         line=dict(color='rgba(0,0,0,0)'), name='Wave Env'), row=1, col=1)
