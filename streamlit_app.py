# Streamlit App: BTC Wave Model + RSI Subplot (Price Visible) + RSI-Only Range Slider
...
def wave_envelope(h,tr):
    ...
    decay  = (1-wcr)**h
    return (tr*10**(decay*(up+wd)),
            tr*10**(decay*(dn-wd)),
            tr*10**(decay*osc))

# 3) Fetch data and build index
...
# 5) Compute trend & envelope arrays
ups, dns, mids, trs = [], [], [], []
for t in idx:
    h  = halving_time(t)
    tr = btc_trend(h)
    up,dn,mid = wave_envelope(h,tr)
    trs.append(tr); ups.append(up); dns.append(dn); mids.append(mid)
...
for lvl,color,op in thresholds:
    ...
    for dt,hot in zip(rsi.index,mask):
        if hot and not in_span:
            ...
            spans.append((start,prev_dt,color,op)); in_span=False
        prev_dt=dt
    if in_span: spans.append((start,prev_dt,color,op))
...
# Main chart row 1
for s,e,c,op in spans:
    fig.add_vrect(x0=s,x1=e,y0=0,y1=1,fillcolor=c,opacity=op,
                  layer="below",row=1,col=1,yref="paper")
fig.add_trace(go.Scatter(x=idx,y=ups, line=dict(color='rgba(0,0,0,0)'),showlegend=False),row=1,col=1)
fig.add_trace(go.Scatter(x=idx,y=dns, fill='tonexty',fillcolor='rgba(0,0,200,0.2)', line=dict(color='rgba(0,0,0,0)'),name='Wave Env'),row=1,col=1)
fig.add_trace(go.Scatter(x=idx,y=trs,mode='lines',line=dict(color='red',dash='dash'),name='Trend'),row=1,col=1)
fig.add_trace(go.Scatter(x=idx,y=mids,mode='lines',line=dict(color='blue'),name='Wave Mid'),row=1,col=1)
fig.add_trace(go.Scatter(x=idx,y=price,mode='lines',line=dict(color='black'),name='Price'),row=1,col=1)
...
for m in midpts:   fig.add_vline(x=m,line=dict(color='gray',dash='dash'))

# RSI subplot row 2
fig.add_trace(go.Scatter(x=rsi.index,y=rsi,mode='lines',line=dict(color='black'),name='RSI'),row=2,col=1)

# 9) Layout: disable primary slider, enable RSI slider only, linear price axis
fig.update_layout(
    height=700,
    hovermode='x unified',
    xaxis=dict(rangeslider=dict(visible=False),type='date'),
    xaxis2=dict(rangeslider=dict(visible=True),type='date'),
    yaxis=dict(title='Price (USD)',fixedrange=False,type='log'),
